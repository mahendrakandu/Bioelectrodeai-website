...
function parse_clinical_cases($file_path) {
    ...
    foreach ($case_matches[1] as $case_text) {
        ...
        $case_obj = [
            ...
            "patientProfile" => trim(str_replace("\n", " ", $get_field("patientProfile"))),
...
if (basename(__FILE__) == basename($_SERVER['SCRIPT_FILENAME'])) {
    $kotlin_file = "c:\\users\\MAHENDRA REDDY\\OneDrive\\Desktop\\Learning\\app\\src\\main\\java\\com\\simats\\learning\\ClinicalCasesActivity.kt";
    $data = parse_clinical_cases($kotlin_file);
    
    $output_file = 'clinical_data.js';
    $js_content = "const clinicalData = " . json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . ";";
